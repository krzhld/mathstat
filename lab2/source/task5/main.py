...
def get_mix_two_dim_normal_sample(size, rho):
    return 0.9 * get_two_dim_normal_sample(size, 0.9) + 0.1 * get_two_dim_normal_sample(size, -0.9, d=[10, 10])
...
def draw_ellipse(size, rhos):
    ...
    str_size = "n = " + str(size)
    titles = [str_size + r', $ \rho = 0$', str_size + r', $\rho = 0.5 $', str_size + r', $ \rho = 0.9$']
    for i in range(len(rhos)):
        ...
        fig.savefig(path.PATH + f'\5_{size}')
    plt.show()
...
if __name__ == '__main__':
    ...
    for size in sizes:
        for ro in rhos:
            pearson, spearman, quadrant = get_correlation_coeffs(get_two_dim_normal_sample, size, ro, repeats)
            print('\n' + str(size) + '\n' + str(create_table(pearson, spearman, quadrant, size, repeats, ro)))

        pearson, spearman, quadrant = get_correlation_coeffs(get_mix_two_dim_normal_sample, size, 0, repeats)
